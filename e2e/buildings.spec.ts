...
test.describe("건물 API", () => {
  test("GET /api/buildings 는 미인증 시에도 목록을 주고 비밀번호는 마스킹", async ({ request }) => {
    const res = await request.get("/api/buildings")
    expect(res.status()).toBe(200)
    const body = (await res.json()) as { buildings?: { password: string }[] }
    expect(Array.isArray(body.buildings)).toBeTruthy()
    if (body.buildings && body.buildings.length > 0) {
      for (const b of body.buildings.slice(0, 5)) {
